# ------------------------------------------------------------
# This script implements an end-to-end ML pipeline for
# predicting wellness tourism package purchases.
#
# Workflow:
# 1. Authenticate and connect to Hugging Face Hub.
# 2. Upload raw CSV data to the Hugging Face Dataset Hub.
# 3. Load the dataset and perform cleaning & preprocessing.
# 4. Split the data into stratified train/test sets and upload them.
# 5. Encode categorical variables for modeling.
# 6. Train an XGBoost classifier on the training set.
# 7. Evaluate the model on the test set.
# 8. Save and register the trained model to Hugging Face Model Hub.
# ------------------------------------------------------------

import pandas as pd
import joblib
import os
from xgboost import XGBClassifier
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import LabelEncoder
from sklearn.metrics import f1_score
from huggingface_hub import HfApi, login

# ------------------------------------------------------------
# 1. Setup & Authentication
# ------------------------------------------------------------
HF_TOKEN = os.getenv("HF_TOKEN")  # Hugging Face API token from environment
REPO_ID = "shashankksaxena"
login(token=HF_TOKEN)
api = HfApi()

# ------------------------------------------------------------
# STEP A: SYNC RAW DATA (SCALABILITY)
# ------------------------------------------------------------
# Upload local raw CSV to Hugging Face Dataset Hub if it exists
if os.path.exists("tourism_project/data/tourism.csv"):
    api.upload_file(
        path_or_fileobj="tourism_project/data/tourism.csv",
        path_in_repo="tourism.csv",
        repo_id=f"{REPO_ID}/tourism-data",
        repo_type="dataset"
    )

# ------------------------------------------------------------
# STEP B: LOAD & CLEAN
# ------------------------------------------------------------
# Load dataset from Hugging Face
data_url = f"https://huggingface.co/datasets/{REPO_ID}/tourism-data/raw/main/tourism.csv"
df = pd.read_csv(data_url)

# Drop identifiers and fix typos
df.drop(columns=['Unnamed: 0', 'CustomerID'], errors='ignore', inplace=True)
df['Gender'] = df['Gender'].replace('Fe Male', 'Female')

# Handle missing values: numeric → median, categorical → mode
for col in df.columns:
    if df[col].dtype == 'object':
        df[col] = df[col].fillna(df[col].mode()[0])
    else:
        df[col] = df[col].fillna(df[col].median())

# ------------------------------------------------------------
# STEP C: SPLIT & UPLOAD DATASETS (RUBRIC REQUIREMENT)
# ------------------------------------------------------------
# Stratified train/test split to maintain target distribution
train_df, test_df = train_test_split(
    df, test_size=0.2, random_state=42, stratify=df['ProdTaken']
)

# Save datasets locally before upload
train_df.to_csv("train.csv", index=False)
test_df.to_csv("test.csv", index=False)

# Upload the resulting train and test sets to Hugging Face Dataset Hub
api.upload_file(path_or_fileobj="train.csv", path_in_repo="train.csv", 
                repo_id=f"{REPO_ID}/tourism-data", repo_type="dataset")
api.upload_file(path_or_fileobj="test.csv", path_in_repo="test.csv", 
                repo_id=f"{REPO_ID}/tourism-data", repo_type="dataset")

print("Train and Test datasets versioned and uploaded to Hugging Face.")

# ------------------------------------------------------------
# STEP D: ENCODING & TRAINING
# ------------------------------------------------------------
# Label encode categorical columns for XGBoost
le = LabelEncoder()
cat_cols = train_df.select_dtypes(include=['object']).columns

for col in cat_cols:
    train_df[col] = le.fit_transform(train_df[col])
    test_df[col] = le.transform(test_df[col])

# Split into features and target
X_train = train_df.drop('ProdTaken', axis=1)
y_train = train_df['ProdTaken']
X_test = test_df.drop('ProdTaken', axis=1)
y_test = test_df['ProdTaken']

# Train XGBoost model
model = XGBClassifier(
    n_estimators=200, 
    max_depth=5, 
    learning_rate=0.1, 
    eval_metric='logloss'
)
model.fit(X_train, y_train)

# ------------------------------------------------------------
# STEP E: EVALUATE & REGISTER MODEL
# ------------------------------------------------------------
# Evaluate model on test set
y_pred = model.predict(X_test)
score = f1_score(y_test, y_pred)
print(f"Model trained. Test F1 Score: {score:.4f}")

# Save model locally and upload to Hugging Face Model Hub
joblib.dump(model, "model.pkl")
api.upload_file(
    path_or_fileobj="model.pkl",
    path_in_repo="model.pkl",
    repo_id=f"{REPO_ID}/tourism-model",
    repo_type="model"
)
print("Best model registered in the Hugging Face Model Hub.")
